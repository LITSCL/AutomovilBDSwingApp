...
public class AutomovilDAO {
	...
	//M�todo que a�ade objetos de tipo Automovil a la tabla automoviles que se encuentra en la base de datos.
	public boolean save(Automovil au) {
		// Todos los m�todos que se conecten a una base de datos deben seguir los 4 siguientes pasos:
				boolean resultado;
				try {
					...
					Connection co = bdUtil.getConexion(); //Esta instrucci�n retorna la conecci�n activa.
					...
					System.out.println("Ejecuci�n del SQL: " + resultado);
					//4. Desconectarse.
				} finally { //Esta instrucci�n se ejecuta se caiga o no el programa.
					bdUtil.desconectar(); //Envia la petici�n de desconecci�n al dbms.
				}
				return resultado;
			}
	
	//M�todo que trae todos los objetos de la tabla llamada automoviles que se encuentra en la base de datos.
	public List<Automovil> getAll() {
		...
		try {
			...
			PreparedStatement st = bdUtil.getConexion().prepareStatement(sql); //Aca se trae la conecci�n y se prepara el statement.
			//3. Ejecutar el SQL.
			...
			while (rs.next()) { //Se repite mientras avance el puntero.
				...
				automoviles.add(au); //A�ade el automovil a la lista (Dicha lista es la que se muestra en la interfaz).
			}
	...
	//M�todo que trae objetos seg�n un filtro de la tabla automoviles que se encuentra en la base de datos.
	...
	public void update(Automovil a) {
		...
		try {
			...
			//4. Desconectarse.
		} finally { //Esta instrucci�n se ejecuta, caiga o no el programa.
			bdUtil.desconectar(); //Envia la petici�n de desconecci�n al dbms.
			resultado = false;
			System.out.println("Ejecuci�n del SQL: " + resultado);
		}
	...
	//M�todo que Elimina objetos de la tabla automoviles que se encuentra en la base de datos.
